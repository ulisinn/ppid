...
class PopUp {
    ...
    private _popupInner:any;
    private _popupOuter:any;
    private _popupWidth:number;

    constructor(customEvt, evtDispatch, selection) {
        ...
        setTimeout(() => this.setPosition(selection, this._mouseEvent), 200)

    }

    createDesktopPopUp(popupInner:any, selection:any) {
        var popupHeight:number = 239,
            popupWidth = 392,
            popupContent;
        ...
        popupInner.selectAll("div.popupContent")
            .data(this._data)
            .enter()
            .append("div")
            .classed("popupContent", true);
    ...
    createPhonePopUp(popupInner:any, selection:any) {
        var popupHeight:number = 360,
            popupWidth = 255,
            popupContent;
    ...
    slidePanel(id, index) {
        //console.log("slidePanel", id, index);
        switch (index) {
            case 0:
    ...
    setPosition(selection, mouseEvent) {
        var whichDevice = this._whichDevice,
            left = mouseEvent.clientX,
            top = mouseEvent.clientY,
            popupContent = selection.select("#popupOuter")[0][0],
            h = popupContent.clientHeight,
            orientation = this.getDeviceOrientation(),
            data = selection.select("#popupOuter").datum();

        if(data.position == "right"){
            left = left - this._popupWidth - 30;
        }
        //console.log(whichDevice, popupContent, h, "set PopUp Position", top, left, mouseEvent);
        selection.select("#popupOuter")
